...
@watch_group.command(name="remove", description="Remove a watchlist entry by id")
...
async def remove(interaction: discord.Interaction, watch_id: int):
    ...
    try:
        ...
        if ok:
            await interaction.followup.send(f"Removed watchlist entry {watch_id}.", ephemeral=True)
        else:
