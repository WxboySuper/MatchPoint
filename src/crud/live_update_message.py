...
def get_live_message(
    session, guild_id: int, scope_type: Optional[str] = None, scope_key: Optional[str] = None
    ...
    """
    stmt = select(LiveUpdateMessage).where(LiveUpdateMessage.guild_id == guild_id)
...
def set_live_message(session, guild_id: int, channel_id: int, message_id: int, scope_type: str = "guild_live", scope_key: Optional[str] = None) -> LiveUpdateMessage:
...
async def get_live_message_async(
    session: AsyncSession, guild_id: int, scope_type: Optional[str] = None, scope_key: Optional[str] = None
) -> Optional[LiveUpdateMessage]:
    stmt = select(LiveUpdateMessage).where(LiveUpdateMessage.guild_id == guild_id)
    if scope_type is not None:
        stmt = stmt.where(LiveUpdateMessage.scope_type == scope_type)
...
async def set_live_message_async(session: AsyncSession, guild_id: int, channel_id: int, message_id: int, scope_type: str = "guild_live", scope_key: Optional[str] = None) -> LiveUpdateMessage:
    rec = await get_live_message_async(session, guild_id, scope_type, scope_key)
...
async def delete_live_message_async(session: AsyncSession, guild_id: int) -> None:
